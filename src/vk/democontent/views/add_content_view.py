...
class AddContentView(BrowserView):
    ...
    def __call__(self):
        ...
        if self.request.method == "POST":
            self.number_of_documents = int(
                self.request.get("number_of_documents", ""))
            self.number_of_news_items = int(
                self.request.get("number_of_news_items", "")
            )
            ...
            self.number_of_folders = int(
                self.request.get("number_of_folders", ""))
            self.max_folder_depth = int(
                self.request.get("max_folder_depth", ""))
    ...
    def create_folders(self):
        while len(self.folder_list) - 1 < self.number_of_folders:
            ...
            depth = len(container.getPhysicalPath()) - len(
    ...
    def get_random_words(self, count=10):
        file_path = pkg_resources.resource_filename(
            "vk.democontent.views", "wortliste.txt"
        )
    ...
    def create_file(self, buffer, suffix, content_type):
        ...
        return NamedBlobFile(
            data=file_data, filename=file_name, contentType=content_type
        )

    def create_file_pdf(self, words_to_use):
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4)
        doc.title = TextLorem(srange=(1, 5), ssep="", words=words_to_use).sentence()[
            :-1
        ]  # ohne Punkt
        story = []
        text = TextLorem(
            srange=(2, 8), prange=(10, 30), trange=(60, 100), words=words_to_use
        ).text()
        ...
        for paragraph_text in paragraphs:
            ...
            # 12 Punkte Abstand zwischen den Absätzen
    ...
    def create_file_docx(self, words_to_use):
        ...
        doc = Document()
        doc.add_heading(
            TextLorem(srange=(1, 5), ssep="",
                      words=words_to_use).sentence()[:-1]
        )
        text = TextLorem(
            srange=(2, 8), prange=(10, 30), trange=(60, 100), words=words_to_use
        ).text()
    ...
    def create_image(self):
        ...
        return NamedBlobImage(
            data=file_data, contentType="image/jpeg", filename=file_name
        )
    ...
    def create_content(self, type, subtype=None, container=None):
        ...
        d = {}
        d["type"] = type
        d["title"] = TextLorem(srange=(1, 5), ssep="", words=words_to_use).sentence()[
            :-1
        ]  # ohne Punkt
